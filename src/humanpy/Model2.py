...
class Data:
    ...
    share_directories = find_in_workspaces(search_dirs=['share'],
                                               project='momdp_data')
    if len(share_directories)==0:
        raise ValueError('Couldnt find momdp data')

    folderName = share_directories[0]+'/'
    momdpOutFolderName = share_directories[0]+'/'
    ...
    def __init__(self, id):
        ...
        self.R = numpy.zeros(
            [self.NUMOFSTATES, self.NUMOFROBOTACTIONS, self.NUMOFHUMANACTIONS, self.NUMOFSTATES])
        self.T = numpy.zeros(
            [self.NUMOFUNOBSSTATES, self.NUMOFSTATES, self.NUMOFROBOTACTIONS, self.NUMOFSTATES])
        self.NUMOFALPHAVECTORS = 7527
        #self.NUMOFALPHAVECTORS = 42
        self.A = numpy.zeros(
            [self.NUMOFALPHAVECTORS, self.NUMOFUNOBSSTATES + 2])
        # assume that the state before last is the starting state. The last one
        ...
        self.stateNames = numpy.asarray(
            [line.split(' ') for line in stateNamesFile])[0]
        ...
        for ra in range(0, self.NUMOFROBOTACTIONS):
            for ha in range(0, self.NUMOFHUMANACTIONS):
                reachFile = open(self.folderName + 'R' +
                                 str(ra + 1) + str(ha + 1) + '.dat', 'r')
                reachMtx = numpy.asarray(
                    [map(float, line.split('\t')) for line in reachFile])
        ...
        for yIndx in range(0, self.NUMOFUNOBSSTATES):
            for ra in range(0, self.NUMOFROBOTACTIONS):
                transFile = open(self.folderName + 'T' +
                                 str(yIndx + 1) + str(ra + 1) + '.dat', 'r')
                transMtx = numpy.asarray(
                    [map(float, line.split('\t')) for line in transFile])
                for ss in range(0, self.NUMOFSTATES):
                    for nss in range(0, self.NUMOFSTATES):
    ...
    def stateUpdateFromHumanAction(self, humanAction, x):
        ...
        nextState = x.getNextStateFromHumanRobotAction(
            self.currState, robotAction, humanAction)
        new_bel_t = x.getNewBeliefFromHumanAction(
            self.currState, robotAction, nextState, self.bel_t)
